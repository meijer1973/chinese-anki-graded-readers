...
The current high-frequency character-compound limit is 2100; `high_frequency_characters_2100.txt` is the expected character file.

Fetch these three files first:

1. `data/external_agent_vocab/high_frequency_characters_2100.txt`
...
2. Check whether every Hanzi character in the token appears in `high_frequency_characters_2100.txt`. If yes, classify the token as `high_frequency_character_compound`.
...
Also track minimum difficulty while drafting. Count every token whose Hanzi characters all appear in the first 500 lines of `high_frequency_characters_2100.txt`; this is `easy_character_compound_token_percent` in repository validation. The hard ceiling is **95%**. As a soft target only, use some compact known words from the upper 50% of the active known-word baseline and some character compounds that need characters from the upper 50% of the enabled character-compound band when they fit the story naturally.
