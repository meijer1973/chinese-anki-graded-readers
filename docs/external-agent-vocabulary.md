...
The current high-frequency character-compound limit is 600; `high_frequency_characters_600.txt` is the expected character file.

Fetch these three files first:

1. `data/external_agent_vocab/high_frequency_characters_600.txt`
...
2. Check whether every Hanzi character in the token appears in `high_frequency_characters_600.txt`. If yes, classify the token as `high_frequency_character_compound`.
