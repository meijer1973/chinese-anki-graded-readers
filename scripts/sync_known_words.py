...
def sync_known_words(source: Path, out: Path, limit: int, metadata: Path | None = None) -> dict:
    ...
    payload = {
        ...
        "limit": limit,
        "derivation": "First N ranked entries from word list chinese.txt. Current graded-reader default N=1500; Anki card scheduling policy is managed separately.",
    }
    if metadata:
...
def main() -> int:
    ...
    parser.add_argument("--limit", type=int, default=1500, help="Number of ranked words to mark known. Use 0 for all words.")
