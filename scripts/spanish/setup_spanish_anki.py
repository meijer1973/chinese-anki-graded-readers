...
from scripts.spanish.validate_spanish_core_100 import (  # noqa: E402
...
DECK_NAME = "Spanish"
MODEL_NAME = "Spanish Vocabulary"
OPTIONS_PRESET_NAME = "Spanish - 5 new cards"
WORD_TEMPLATE_NAME = "Word Recognition"
SENTENCE_TEMPLATE_NAME = "Sentence Recognition"
STANDARD_TAGS = {"spanish", "spanish::core_100"}

CHINESE_DECK_NAME = "Default"
CHINESE_DECK_QUERY = "deck:Default"
CHINESE_MODEL_NAME = "Chinese Vocabulary"
HINDI_DECK_NAME = "Hindi"
HINDI_DECK_QUERY = "deck:Hindi"
HINDI_MODEL_NAME = "Hindi Vocabulary"

REPORTS_DIR = ROOT / "anki" / "spanish" / "reports"
DRY_RUN_REPORT = REPORTS_DIR / "spanish_anki_dry_run.json"
APPLY_REPORT = REPORTS_DIR / "spanish_anki_apply_report.json"
VERIFICATION_REPORT = REPORTS_DIR / "spanish_anki_verification.json"
...
HINDI_SAFETY_BEFORE = REPORTS_DIR / "hindi_safety_before.json"
HINDI_SAFETY_AFTER = REPORTS_DIR / "hindi_safety_after.json"
HINDI_SAFETY_COMPARISON = REPORTS_DIR / "hindi_safety_comparison.json"
SPANISH_BACKUP_TSV = REPORTS_DIR / "spanish_managed_notes_before_update.tsv"
...
WORD_FRONT = """
<main class="spanish-card word-card">
  <div class="word" lang="es">{{Word}}</div>
</main>
""".strip()
...
<section class="answer-primary">
  ...
  {{#Lemma}}<div class="lemma"><span>Lemma</span> {{Lemma}}</div>{{/Lemma}}
</section>
{{#Example}}
<section class="example-section">
  <div class="example" lang="es">{{Example}}</div>
  <div class="example-ipa">{{Example IPA}}</div>
...
SENTENCE_FRONT = """
{{#Example}}
<main class="spanish-card sentence-card">
  <div class="example sentence-front" lang="es">{{Example}}</div>
</main>
{{/Example}}
""".strip()
...
<section class="target-word-section">
  ...
  <div class="target-word" lang="es">{{Word}}</div>
  <div class="ipa">{{IPA}}</div>
  <div class="meaning">{{Meaning}}</div>
  {{#Lemma}}<div class="lemma"><span>Lemma</span> {{Lemma}}</div>{{/Lemma}}
  {{#Part of Speech}}<div class="part-of-speech">{{Part of Speech}}</div>{{/Part of Speech}}
...
PRODUCTION_LIKE_RE = re.compile(
    r"production|recall|reverse|english\s*(?:→|->|to)\s*spanish|sentence\s+production",
    re.IGNORECASE,
)


class SpanishSetupError(RuntimeError):
    """Raised when the live Spanish resources are incompatible or incomplete."""
...
class SpanishAnkiSetup:
    def __init__(
        ...
        *,
        ...
        self.spanish_only_option_changes: list[str] = []
    ...
    def protected_snapshot(
        ...
        *,
        ...
        notes = self.find_notes(deck_query)
        cards = self.find_cards(deck_query)
        ...
        if model_name in model_names:
            ...
            protected_model = {"exists": False, "fields": [], "templates": {}, "styling": {}}
        ...
        return {
            ...
            "deck_id": int(self.client.read("deckNamesAndIds").get(deck_name, -1)),
            "note_ids": [int(note["noteId"]) for note in notes],
            "card_ids": [int(card["cardId"]) for card in cards],
            "notes": normalized_notes,
            "cards": normalized_cards,
            "note_type": protected_model,
            "deck_config": self.client.read("getDeckConfig", {"deck": deck_name}),
        }

    def chinese_snapshot(self) -> dict[str, Any]:
        return self.protected_snapshot(
            deck_name=CHINESE_DECK_NAME,
            deck_query=CHINESE_DECK_QUERY,
            model_name=CHINESE_MODEL_NAME,
        )

    def hindi_snapshot(self) -> dict[str, Any]:
        return self.protected_snapshot(
            deck_name=HINDI_DECK_NAME,
            deck_query=HINDI_DECK_QUERY,
            model_name=HINDI_MODEL_NAME,
        )
    ...
    def install_guard(
        self,
        chinese_snapshot: dict[str, Any],
        hindi_snapshot: dict[str, Any],
    ) -> MutationGuard:
        protected_snapshots = [chinese_snapshot, hindi_snapshot]
        protected_config_ids = {
            int(snapshot["deck_config"]["id"])
            for snapshot in protected_snapshots
            if (snapshot.get("deck_config") or {}).get("id") is not None
        }
        guard = MutationGuard(
            spanish_deck=DECK_NAME,
            spanish_model=MODEL_NAME,
            spanish_preset=OPTIONS_PRESET_NAME,
            protected_decks={CHINESE_DECK_NAME, HINDI_DECK_NAME},
            protected_models={CHINESE_MODEL_NAME, HINDI_MODEL_NAME},
            protected_note_ids={
                int(value)
                for snapshot in protected_snapshots
                for value in snapshot.get("note_ids", [])
            },
            protected_card_ids={
                int(value)
                for snapshot in protected_snapshots
                for value in snapshot.get("card_ids", [])
            },
    ...
    def inspect_spanish(self) -> dict[str, Any]:
    ...
    def assert_compatible(self, state: dict[str, Any]) -> None:
        problems: list[str] = []
        if state["subdecks"]:
            problems.append(f"Spanish subdecks are not managed by this setup: {state['subdecks']}")
        if state["model_exists"]:
            if state["fields"] != FIELDS:
                problems.append(f"existing Spanish Vocabulary fields are incompatible: {state['fields']}")
            if set(state["templates"]) != {WORD_TEMPLATE_NAME, SENTENCE_TEMPLATE_NAME}:
                problems.append(
                    "existing Spanish Vocabulary must have exactly Word Recognition and Sentence Recognition templates"
                )
        ...
        if outside_deck:
            problems.append(f"Spanish Vocabulary notes exist outside deck Spanish: {sorted(outside_deck)[:10]}")

        seen_words: set[str] = set()
        for note in state["notes"]:
            note_id = int(note["noteId"])
            if note.get("modelName") != MODEL_NAME:
                problems.append(f"deck Spanish contains note {note_id} on unrelated type {note.get('modelName')!r}")
                continue
            word = note_field(note, "Word")
            if word not in self.rows_by_word:
                problems.append(f"deck Spanish contains unmanaged word {word!r} on note {note_id}")
            if word in seen_words:
                problems.append(f"deck Spanish contains duplicate managed word {word!r}")
            seen_words.add(word)

        if state["cards"] and not state["model_exists"]:
            problems.append("deck Spanish contains cards but Spanish Vocabulary does not exist")
        if problems:
            raise SpanishSetupError("; ".join(problems))

    def row_fields(self, row: dict[str, str]) -> dict[str, str]:
        fields = {field: row[field] for field in FIELDS}
        if row["Word"].casefold() == row["Lemma"].casefold():
            fields["Lemma"] = ""
        return fields
    ...
    def plan(
        ...
        return {
            ...
            "proposed_mutations": {
                ...
                "spanish_only_option_keys_to_change": option_changes,
            ...
            "protected_config_ids": sorted(protected_config_ids),
        }

    def dry_run(self) -> dict[str, Any]:
        mutation_count_before = len(self.client.mutation_log)
        chinese_snapshot = self.chinese_snapshot()
        hindi_snapshot = self.hindi_snapshot()
        state = self.inspect_spanish()
        plan = self.plan(state, chinese_snapshot, hindi_snapshot)
        ...
        report = {
            ...
            "hindi_safety_sha256": sha256_json(hindi_snapshot),
            "current_spanish_state": {
        ...
        if report["mutations_performed"] != 0:
            raise SpanishSetupError("dry-run performed a mutation")
    ...
    def ensure_deck(self, state: dict[str, Any]) -> None:
        if not state["deck_exists"]:
            ...
            if not deck_id:
                raise SpanishSetupError("AnkiConnect did not create deck Spanish")
    ...
    def ensure_model(self, state: dict[str, Any]) -> None:
        if not state["model_exists"]:
            ...
            if not result:
                raise SpanishSetupError("AnkiConnect did not create Spanish Vocabulary")
            return
    ...
    def ensure_options(
        self,
        protected_configs_before: dict[str, dict[str, Any]],
    ) -> dict[str, Any]:
        if self.client.guard is None:
            raise SpanishSetupError("mutation guard is missing")
        inherited = self.client.read("getDeckConfig", {"deck": DECK_NAME})
        if not inherited:
            raise SpanishSetupError("could not inspect the options preset inherited by deck Spanish")
        ...
        if inherited.get("name") == OPTIONS_PRESET_NAME and inherited_id not in protected_ids:
            ...
            if cloned is False or cloned is None:
                raise SpanishSetupError(
                    "AnkiConnect cannot safely clone an independent Spanish options preset; apply aborted"
                )
            spanish_config_id = int(cloned)
            if spanish_config_id in protected_ids:
                raise SpanishSetupError(
                    "cloned Spanish preset unexpectedly reused a protected Chinese/Hindi preset"
                )
            self.client.guard.register_spanish_config(spanish_config_id)
            assigned = self.client.mutate(
                "setDeckConfigId",
                {"decks": [DECK_NAME], "configId": spanish_config_id},
            )
        ...
        if "new" not in updated or "perDay" not in updated["new"]:
            raise SpanishSetupError("installed Anki options do not expose new.perDay safely")
        ...
        for deck_name, before_config in protected_configs_before.items():
            current_config = self.client.read("getDeckConfig", {"deck": deck_name})
            if current_config != before_config:
                raise SpanishSetupError(
                    f"{deck_name} options changed while creating the Spanish preset"
                )
        return final

    def write_spanish_backup(self, notes: list[dict[str, Any]]) -> None:
        if not notes:
            return
        path = self.reports_dir / SPANISH_BACKUP_TSV.name
        path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def ensure_notes(self, existing_notes: list[dict[str, Any]]) -> dict[str, int]:
        if self.client.guard is None:
            raise SpanishSetupError("mutation guard is missing")
        self.write_spanish_backup(existing_notes)
        self.client.guard.register_spanish_notes({int(note["noteId"]) for note in existing_notes})
        ...
        for start in range(0, len(update_actions), 50):
            ...
            for result in results:
                if isinstance(result, dict) and result.get("error"):
                    raise SpanishSetupError(f"Spanish field update failed: {result['error']}")
        ...
        if missing_rows:
            ...
            if len(results) != len(notes) or any(result is None for result in results):
                raise SpanishSetupError(f"addNotes did not create all Spanish notes: {results}")
            added_ids = [int(result) for result in results]
            self.client.guard.register_spanish_notes(added_ids)

        current_notes = self.find_notes(f"deck:{DECK_NAME}")
        self.client.guard.register_spanish_notes({int(note["noteId"]) for note in current_notes})
        tag_groups: dict[str, list[int]] = defaultdict(list)
        for note in current_notes:
            row = self.rows_by_word.get(note_field(note, "Word"))
            if row is None:
                raise SpanishSetupError("an unmanaged Spanish note appeared during apply")
    ...
    def ensure_card_order(self) -> dict[str, Any]:
        ...
        self.client.guard.register_spanish_notes({int(note["noteId"]) for note in notes})
        self.client.guard.register_spanish_cards({int(card["cardId"]) for card in cards})
        ...
        try:
            ...
            for due, card_id in enumerate(expected, start=1):
                ...
                if not result or result[0] is not True:
                    raise SpanishSetupError(f"due-field order fallback failed for card {card_id}: {result}") from exc
        ...
        if self.current_new_card_order(after) != expected:
            raise SpanishSetupError("Spanish new-card order did not match Frequency Rank and template order")
    ...
    def verify_live(self) -> dict[str, Any]:
        ...
        errors: list[str] = []
        if not state["deck_exists"]:
            errors.append("deck Spanish does not exist")
        if not state["model_exists"]:
            errors.append("note type Spanish Vocabulary does not exist")
        ...
        if len(notes_by_word) != 100 or set(notes_by_word) != set(self.rows_by_word):
            errors.append("live Spanish words do not match the 100 TSV source words")
        if any(note.get("modelName") != MODEL_NAME for note in notes):
            errors.append("deck Spanish contains a note on another note type")
        ...
        if notes_with_standard_tags != 100:
            errors.append("not every Spanish note has both standard Spanish tags")
        if notes_with_pos_tag != 100:
            errors.append("not every Spanish note has a normalized part-of-speech tag")
        ...
        chinese_config = self.client.read("getDeckConfig", {"deck": CHINESE_DECK_NAME})
        hindi_config = self.client.read("getDeckConfig", {"deck": HINDI_DECK_NAME})
        protected_config_ids = {
            int(value.get("id", -1))
        ...
        if not config:
            ...
            if int(config.get("newGatherPriority", -1)) != 0:
                errors.append("Spanish new-card gather order is not deck/ascending-position order")
            if int(config.get("newSortOrder", -1)) != 1:
                errors.append("Spanish new-card sort order is not order gathered")
        ...
        report = {
            ...
            "chinese_config_id": chinese_config.get("id")
            if isinstance(chinese_config, dict)
            else None,
            "hindi_config_id": hindi_config.get("id")
            if isinstance(hindi_config, dict)
            else None,
            "separate_options_preset": bool(config)
            and int(config.get("id", -1)) not in protected_config_ids,
    ...
    def apply(self) -> dict[str, Any]:
        chinese_before = self.chinese_snapshot()
        hindi_before = self.hindi_snapshot()
        chinese_before_doc = self.snapshot_document(chinese_before)
        hindi_before_doc = self.snapshot_document(hindi_before)
        write_json(self.reports_dir / CHINESE_SAFETY_BEFORE.name, chinese_before_doc)
        write_json(self.reports_dir / HINDI_SAFETY_BEFORE.name, hindi_before_doc)
        guard = self.install_guard(chinese_before, hindi_before)
        report: dict[str, Any] = {
            "status": "FAIL",
            "mode": "apply",
            "deck": DECK_NAME,
            "note_type": MODEL_NAME,
            "options_preset": OPTIONS_PRESET_NAME,
            "chinese_safety_before_sha256": chinese_before_doc["sha256"],
            "hindi_safety_before_sha256": hindi_before_doc["sha256"],
        }
        try:
            state = self.inspect_spanish()
            self.assert_compatible(state)
            guard.register_spanish_notes({int(note["noteId"]) for note in state["notes"]})
            guard.register_spanish_cards({int(card["cardId"]) for card in state["cards"]})
            plan = self.plan(state, chinese_before, hindi_before)
            self.ensure_deck(state)
            self.ensure_model(state)
            final_config = self.ensure_options(
                {
                    CHINESE_DECK_NAME: chinese_before["deck_config"],
                    HINDI_DECK_NAME: hindi_before["deck_config"],
                }
            )
            note_result = self.ensure_notes(state["notes"])
            order_result = self.ensure_card_order()

            chinese_after = self.chinese_snapshot()
            hindi_after = self.hindi_snapshot()
            chinese_after_doc = self.snapshot_document(chinese_after)
            hindi_after_doc = self.snapshot_document(hindi_after)
            write_json(self.reports_dir / CHINESE_SAFETY_AFTER.name, chinese_after_doc)
            write_json(self.reports_dir / HINDI_SAFETY_AFTER.name, hindi_after_doc)
            chinese_comparison = {
                "before_sha256": chinese_before_doc["sha256"],
                "after_sha256": chinese_after_doc["sha256"],
                "identical": chinese_before_doc["sha256"] == chinese_after_doc["sha256"],
                "structured_equal": chinese_before == chinese_after,
                "differing_top_level_keys": sorted(
                    key
                    for key in set(chinese_before) | set(chinese_after)
                    if chinese_before.get(key) != chinese_after.get(key)
                ),
            }
            hindi_comparison = {
                "before_sha256": hindi_before_doc["sha256"],
                "after_sha256": hindi_after_doc["sha256"],
                "identical": hindi_before_doc["sha256"] == hindi_after_doc["sha256"],
                "structured_equal": hindi_before == hindi_after,
                "differing_top_level_keys": sorted(
                    key
                    for key in set(hindi_before) | set(hindi_after)
                    if hindi_before.get(key) != hindi_after.get(key)
                ),
            }
            write_json(
                self.reports_dir / CHINESE_SAFETY_COMPARISON.name,
                chinese_comparison,
            )
            write_json(
                self.reports_dir / HINDI_SAFETY_COMPARISON.name,
                hindi_comparison,
            )
            if (
                not chinese_comparison["identical"]
                or not chinese_comparison["structured_equal"]
            ):
                raise SpanishSetupError("Chinese snapshot changed during Spanish apply")
            if not hindi_comparison["identical"] or not hindi_comparison["structured_equal"]:
                raise SpanishSetupError("Hindi snapshot changed during Spanish apply")

            verification = self.verify_live()
            write_json(self.reports_dir / VERIFICATION_REPORT.name, verification)
            if verification["status"] != "PASS":
                raise SpanishSetupError("; ".join(verification["errors"]))
            ...
            report.update(
                {
                    ...
                    "spanish_only_option_keys_changed": self.spanish_only_option_changes,
                    "options_config_id": final_config["id"],
                    "new_cards_per_day": final_config["new"]["perDay"],
                    "chinese_safety_after_sha256": chinese_after_doc["sha256"],
                    "chinese_safety_identical": True,
                    "hindi_safety_after_sha256": hindi_after_doc["sha256"],
                    "hindi_safety_identical": True,
            ...
            try:
                ...
                report["chinese_safety_after_sha256"] = chinese_after_doc["sha256"]
                report["chinese_safety_identical"] = chinese_comparison["identical"]
                report["hindi_safety_after_sha256"] = hindi_after_doc["sha256"]
                report["hindi_safety_identical"] = hindi_comparison["identical"]
    ...
    def verify_only(self) -> dict[str, Any]:
        ...
        if report["status"] != "PASS":
            raise SpanishSetupError("; ".join(report["errors"]))
        return report
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Safely create, update, or verify the isolated Spanish Core 100 Anki deck."
    )
    modes = parser.add_mutually_exclusive_group()
    modes.add_argument("--dry-run", action="store_true", help="Inspect and report proposed Spanish-only changes (default).")
    modes.add_argument("--apply", action="store_true", help="Apply guarded Spanish-only mutations.")
    modes.add_argument("--verify-only", action="store_true", help="Inspect the existing live Spanish resources without mutation.")
...
def main() -> int:
    ...
    except (AnkiConnectError, SpanishSetupError, MutationSafetyError, OSError, ValueError) as exc:
