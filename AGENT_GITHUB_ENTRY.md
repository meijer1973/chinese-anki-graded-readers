...
| What should external writer agents download for quick vocabulary preflight? | `docs/external-agent-vocabulary.md`, `data/external_agent_vocab/high_frequency_characters_2100.txt`, `data/external_agent_vocab/known_words_minus_character_compounds.txt`, `data/external_agent_vocab/master_stretch_words_non_core.txt`, `data/external_agent_vocab/metadata.json` |
...
| How are selected First Frost character word cards enabled? | `docs/first-frost-character-pilot.md`, `anki/first_frost/character_pilot.json`, `scripts/first_frost/character_pilot.py`; sentence siblings remain suspended |
...
- Missing the extensive-reading vocabulary gates: default validation requires at least 98% known tokens, at most 2% approved non-core tokens, and at most 95% first-500 character-compound tokens.
- Forgetting to pass stretch packs, `book_specific_words.txt`, and `proper_nouns.txt` when validating layered manuscripts.
- Forgetting `--known-character-compounds --known-character-compound-limit 2100` for Marcel personalized manuscripts that use the high-frequency character-compound layer.
