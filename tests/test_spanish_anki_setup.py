...
from scripts.spanish.anki_client import AnkiClient
...
class SpanishAnkiSetupTests(unittest.TestCase):
    ...
    def make_manager(self, fake: FakeAnki, reports_dir: Path) -> SpanishAnkiSetup:
    ...
    def test_apply_creates_exact_counts_and_targets_spanish_only(self) -> None:
        fake = FakeAnki()
        original_chinese_note = copy.deepcopy(fake.notes[10])
        original_chinese_card = copy.deepcopy(fake.cards[20])
        original_hindi_note = copy.deepcopy(fake.notes[11])
        original_hindi_card = copy.deepcopy(fake.cards[21])
        with tempfile.TemporaryDirectory() as tmp:
        ...
        self.assertEqual(original_hindi_note, fake.notes[11])
        self.assertEqual(original_hindi_card, fake.cards[21])
    ...
    def test_existing_compatible_note_updates_fields_without_replacing_cards(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            spanish_notes = [note for note in fake.notes.values() if note["deckName"] == DECK_NAME]
            target = next(note for note in spanish_notes if note["fields"]["Word"]["value"] == "de")
            original_note_id = target["noteId"]
            original_card_ids = list(target["cards"])
            target["fields"]["IPA"]["value"] = "wrong"
        ...
        self.assertEqual("/de/", updated["fields"]["IPA"]["value"])
    ...
    def test_options_preset_is_cloned_with_only_required_behavior_changes(self) -> None:
        ...
        hindi_before = copy.deepcopy(fake.configs[2])
        with tempfile.TemporaryDirectory() as tmp:
            report = self.make_manager(fake, Path(tmp)).apply()
        spanish_config_id = fake.deck_config_ids[DECK_NAME]
        spanish = fake.configs[spanish_config_id]
        self.assertNotEqual(1, spanish_config_id)
        self.assertEqual(OPTIONS_PRESET_NAME, spanish["name"])
        self.assertEqual(5, spanish["new"]["perDay"])
        self.assertEqual(1, spanish["newSortOrder"])
        self.assertEqual(0, spanish["newGatherPriority"])
        self.assertEqual(default_before, fake.configs[1])
        self.assertEqual(hindi_before, fake.configs[2])
        self.assertEqual(default_before["new"]["order"], spanish["new"]["order"])
        self.assertEqual(default_before["rev"], spanish["rev"])
        self.assertEqual(default_before["lapse"], spanish["lapse"])
        self.assertEqual(["new.perDay", "newSortOrder"], report["spanish_only_option_keys_changed"])
